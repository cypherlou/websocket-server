...
app = Flask(str(config.get("APPLICATION_NAME", "gwss")))
app.secret_key = str(config.get("SESSION_KEY"))
...
handler.setFormatter(logging.Formatter(str(config["LOG_FORMAT"])))
...
if config.get("ENABLE_REDIS"):
    ...
    __redis = redis.StrictRedis(
        ...
        port=int(str(config["REDIS_PORT"])),
        db=int(str(config["REDIS_DB"])),
    )
...
if config.get("ENABLE_CELERY"):
    ...
    __celery = celery.Celery(
        ...
        backend=config.get("CELERY_BROKER"),
    )
    __celery.conf.update(
        ...
        BROKER_TRANSPORT_OPTIONS={
            "visibility_timeout": config.get("CELERY_VISIBILITY_TIMEOUT")
        },
    )
    app.logger.debug("Celery client enabled")

# SocketIO Wrapper
caos = str(config.get("SOCKETIO_CORS_ALLOWED_ORIGINS", "")).replace(" ", "").split(",")
for cao in caos:
...
def static_file(directory, file_name, file_type):
    ...
    return send_from_directory(
        config["STATIC_FOLDER"] + "/" + directory, "%s.%s" % (file_name, file_type)
    ), {
        "Content-Type": config["MIME_MAPPING"].get(
